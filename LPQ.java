...
public class LPQ {
    ...
    public void dijkstra(Graph graph, String src) {
        ...
        for(int x = 0; x<graph.adj_list.size();x++){

            for(int y=0; y<graph.adj_list.get(x).getList().size(); y++){
                graph2.adj_list.get(x).getList().remove(y);
            }
            for(int z=0; z<graph.adj_list.get(x).getList().size(); z++){
                Graph.Node copy = (Graph.Node) graph.adj_list.get(x).getList().get(z);
                graph2.adj_list.get(x).getList().add(new Graph.Node(copy.getValue(), copy.getWeight() * -1));

            }
        }
        int c = 0;
        for (Graph.VertexNode vn : graph2.adj_list) {
            if (vn.getName().equals(src)) {
                index = c;
            }
            c++;
        }
        List<List<Graph.Node>> adj = graph2.adj_list.get(index).getList();
